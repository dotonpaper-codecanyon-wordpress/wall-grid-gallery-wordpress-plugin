    ...
    if (!class_exists("DOPWallGridGalleryBackEnd")){
        class DOPWallGridGalleryBackEnd{
            ...
            function addWPAdminStyles(){
                // Register Styles.
                wp_register_style('DOPWGG_WPAdminStyle', plugins_url('assets/gui/css/backend-wp-admin-style.css', __FILE__));

                // Enqueue Styles.
                wp_enqueue_style('DOPWGG_WPAdminStyle');
            }
            ...
            function addScripts(){
                // Register JavaScript.
                wp_register_script('DOPWGG_SwfJS', plugins_url('libraries/js/swfobject.js', __FILE__), array('jquery'), false, true);
                wp_register_script('DOPWGG_UploadifyJS', plugins_url('libraries/js/jquery.uploadify.min.js', __FILE__), array('jquery'), false, true);
                wp_register_script('DOPWGG_JcropJS', plugins_url('libraries/js/jquery.Jcrop.min.js', __FILE__), array('jquery'), false, true);
                wp_register_script('DOPWGG_ColorPickerJS', plugins_url('libraries/js/colorpicker.js', __FILE__), array('jquery'), false, true);
                wp_register_script('DOPWGG_DOPImageLoaderJS', plugins_url('libraries/js/jquery.dop.ImageLoader.min.js', __FILE__), array('jquery'), false, true);
                wp_register_script('DOPWGG_DOPWGGJS', plugins_url('assets/js/dopwgg-backend.js', __FILE__), array('jquery'), false, true);
            ...
            function initTables(){// Tables init.
                //update_option('DOPWGG_db_version', '1.0');
                ...
                switch ($images_action){
                    case 'delete':
                        for ($i=0; $i<count($images_list); $i++){
                            $image = $wpdb->get_row('SELECT * FROM '.DOPWGG_Images_table.' WHERE id="'.$images_list[$i].'"');
                            $position = $image->position;

                            $wpdb->query('DELETE FROM '.DOPWGG_Images_table.' WHERE id="'.$images_list[$i].'"');
                            unlink(DOPWGG_Plugin_AbsPath.'uploads/'.$image->name);
                            unlink(DOPWGG_Plugin_AbsPath.'uploads/thumbs/'.$image->name);
                    ...
                    case 'enable':
                        for ($i=0; $i<count($images_list); $i++){
                            $wpdb->update(DOPWGG_Images_table, array('enabled' => 'true'), array('id' => $images_list[$i]));
                        }
                        break;
                    case 'disable':
                        for ($i=0; $i<count($images_list); $i++){
                            $wpdb->update(DOPWGG_Images_table, array('enabled' => 'false'), array('id' => $images_list[$i]));
                        }
                        break;
                }
